    ...
    const fetchHospitalData = async () => {
      ...
      try {
        ...
        const [profileRes, stockRes, requestsRes] = await Promise.all([
          ...
          axios.get(`${baseUrl}/inventory/by-location`, { withCredentials: true }),
          axios.get(`${baseUrl}/inventory/demand`, { withCredentials: true })
        ]);
        ...
        const stockRaw = stockRes.data?.data || stockRes.data || [];
        const requestsRaw = requestsRes.data?.data || requestsRes.data || [];

        const stockData = stockRaw.map(item => ({
          _id: item.bloodtype || item.bloodGroup,
          bloodGroup: item.bloodtype || item.bloodGroup,
          quantity: parseInt(item.totalunits || item.quantity || 0),
          expiryDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString() // Mock expiry for now
        }));

        const requestsData = requestsRaw.map((item, idx) => ({
          _id: idx,
          bloodType: item.bloodtype || item.bloodGroup || 'Unknown',
          units: item.demandcount || item.units || 0,
          status: "pending",
          labId: { name: "System Demand" }
        }));

        const totalUnits = stockData.reduce((sum, item) => sum + (item.quantity || 0), 0);
        ...
        setHospital({
          ...
          type: profileData.position,
          category: profileData.shifttiming,
          address: profileData.locationname || "Location not assigned",
          status: "Active",
          operatingHours: profileData.capacity ? `Capacity: ${profileData.capacity}` : "",
          history: [],
